...
// process lods
	if (bResult&&!l_lods.empty())
    {
        ...
        if (1==ImageLib.CreateMergedTexture	(2,images,merged_image,4096,6144,4096,6144,offsets,scales,rotated,remap)){
            ...
            ELog.DlgMsg		(mtError,"Failed to build merged LOD texture. Merged texture more than [6144x6144].");
        	bResult			= FALSE;
        }
        UI->ProgressEnd(pb);
    }
